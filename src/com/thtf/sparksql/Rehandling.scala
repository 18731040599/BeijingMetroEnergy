...
/**
 * 重新计算处理某个时间的数据，可以在提交任务时为main方法传入的参数（时间）
 */
object Rehandling {
  def main(args: Array[String]): Unit = {
    ...
    val endTime = args(0)
    val startTime = (endTime.toLong - 2000).toString()
    val lastDay = (endTime.toLong - 1000000).toString()
    ...
    def finalHtableToDF(tablename: String) = {
      ...
      conf.set("hbase.zookeeper.property.clientPort", settings.getProperty("hbase.zookeeper.property.clientPort"))
      conf.set("hbase.zookeeper.quorum", settings.getProperty("hbase.zookeeper.quorum"))
      ...
      val tableRDD = spark.sparkContext.newAPIHadoopRDD(conf, classOf[TableInputFormat],
        ...
        .map(result => {
          ...
          for (i <- 0 until columns.length) {
            row = Row.merge(row, Row(Bytes.toString(result.getValue(columnFamily.getBytes, columns(i).getBytes))))
          }
          row
        })
    ...
    def hisdataYCToDF(tablename: String) = {
      ...
      val tableRDD = spark.sparkContext.newAPIHadoopRDD(conf, classOf[TableInputFormat],
        ...
        .map(result => {
          ...
          for (i <- 0 to 599) {
            val value = Bytes.toString(result.getValue(columnFamily.getBytes, i.toString().getBytes))
            if (value == null) {
    ...
    def hisdataYXToDF(tablename: String) = {
      ...
      val tableRDD = spark.sparkContext.newAPIHadoopRDD(conf, classOf[TableInputFormat],
        ...
        .map(result => {
          ...
          Row(str.replaceAll("[0-9]", ""), str.replaceAll("[^0-9]", "").takeRight(14), str.replaceAll("[^0-9]", "").dropRight(14), Bytes.toString(result.getValue(columnFamily.getBytes, "value".getBytes)))
        })
      // 以编程的方式指定 Schema，将RDD转化为DataFrame
      ...
      val tableRDD = spark.sparkContext.newAPIHadoopRDD(conf, classOf[TableInputFormat],
        ...
        .map(result => {
          ...
          for (i <- 0 until result.size()) {
            row = Row.merge(row, Row(Bytes.toString(result.getValue(columnFamily.getBytes, fields(i + 2).getBytes))))
          }
          row
        })
      // 以编程的方式指定 Schema，将RDD转化为DataFrame
    ...
    def saveToHbase(tablename: String, KPI: ArrayBuffer[Array[String]]) {
      ...
      val KPIRdd_real = spark.sparkContext.parallelize(KPI).map(arr => {
        ...
        for (i <- 0 until arr.length - 1) {
          p.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(KPI_fields(i + startnum)), Bytes.toBytes(arr(i + 1)))
        }
    ...
    try {
      ...
      if (lines_df.count() == 0) {
    	  // 如果没有符合要求的站点，直接结束程序
    	  println("There are no stations to process !")
    	  return
      } else {
